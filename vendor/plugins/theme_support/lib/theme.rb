class Theme
  ...
  def self.add_theme_ruby_paths
    self.find_all.each do |theme|
      #theme specific models
      path = File.join(path_to_theme(theme.name),'models')
      $: << path  if File.directory?(path)
      #theme specific helpers.. must be unique per theme
      path = File.join(path_to_theme(theme.name),'helpers')
      $: << path if File.directory?(path)
    end
  end
